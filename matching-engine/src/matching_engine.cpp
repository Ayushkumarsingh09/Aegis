...
void MatchingEngine::set_event_callback(EventCallback cb) { callback_ = std::move(cb); }

OrderId MatchingEngine::next_order_id() { return order_id_gen_++; }

TradeId MatchingEngine::next_trade_id() { return trade_id_gen_++; }

SequenceNum MatchingEngine::next_sequence() { return ++sequence_; }
...
void MatchingEngine::match_order(uint32_t taker_idx, std::vector<OrderEvent>& events) {
  ...
  while (taker.remaining_qty > 0) {
    ...
    uint32_t maker_idx = book_.front_order(taker.side == Side::Buy ? Side::Sell : Side::Buy,
                                           *contra_price);
    ...
    taker.status = taker.remaining_qty == 0 ? OrderStatus::Filled : OrderStatus::PartiallyFilled;
